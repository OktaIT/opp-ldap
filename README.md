...
Big Thanks to Tech Marketing for writing most of the setup documentation.
...
- Okta doesn't store passwords. So once you assign the app to a user in Okta (assuming that Sync Okta Password is enabled), the user will have to change their password so that it can get pushed to the LDAP.
	- The connector removes the password from the SCIMUser object after it creates the attributes to insert into the LDAP.
